  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      setFormData({
        ...
        completedItems: safeParse(found.completedItems),
        missingItems: safeParse(found.missingItems),
        goals: safeParse(found.goals)
      });
    }
  }, [projectId, projects]);
          ...
          <section className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-6 mb-8">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-black">Strategic Project Goals</h2>
              </div>
              <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-full uppercase tracking-widest">Progress Drivers</span>
            </div>

            <div className="space-y-4 mb-8">
              {formData.goals?.length ? formData.goals.map((goal: any, idx: number) => (
                  ...
                  <div className="flex items-center gap-4 flex-grow">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updated[idx] = { ...updated[idx], isCompleted: e.target.checked };
                    ...
                    <input
                      ...
                      className="bg-transparent border-none outline-none font-bold text-slate-700 flex-grow"
                    />
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="text-red-400 opacity-0 group-hover:opacity-100 p-2 hover:bg-red-50 rounded-lg transition-all"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
            ...
            <div className="flex gap-4">
              <button
                onClick={() => {
                  const newGoal = { id: `goal-${Date.now()}`, description: 'New project goal', isCompleted: false };
                  setFormData({ ...formData, goals: [...(formData.goals || []), newGoal] });
                }}
                className="w-full py-4 border-2 border-dashed border-slate-200 rounded-xl text-xs font-black uppercase tracking-widest text-slate-400 hover:border-blue-400 hover:text-blue-600 transition-all flex items-center justify-center gap-2"
              >
                <Plus size={16} /> Add Critical Project Goal
              </button>
            </div>
          </section>
