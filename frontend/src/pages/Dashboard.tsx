...
import type { Project, Donation, Expense, NewsUpdate, GalleryItem, User, Volunteer, AppEvent } from '../types';
...
export const Dashboard = () => {
  ...
  const { currentUser, logout, registerStaff } = useAuth();
  ...
  const [registerForm, setRegisterForm] = useState({ name: '', email: '', role: 'helper' as 'admin' | 'helper', password: '' });
  ...
  const handleStaffRegister = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin) return;
    registerStaff(registerForm.name, registerForm.email, registerForm.role, registerForm.password);
    setIsRegisterModalOpen(false);
    setRegisterForm({ name: '', email: '', role: 'helper', password: '' });
  };
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <form onSubmit={handleStaffRegister} className="p-12 space-y-10">
              ...
              <div className="space-y-3">
                ...
                <input required type="password" placeholder="••••••••" className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-black text-lg" value={registerForm.password} onChange={(e) => setRegisterForm({ ...registerForm, password: e.target.value })} />
              </div>
