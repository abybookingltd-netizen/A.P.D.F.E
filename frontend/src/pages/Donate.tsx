...
import { CreditCard, Smartphone, Building, ShieldCheck, Heart, CheckCircle } from 'lucide-react';
...
export const Donate = () => {
  ...
  const handleDonate = async (e: React.FormEvent) => {
    ...
    try {
      await addDonation({
        ...
        source: isMonthly ? 'Recurring' : 'Individual',
        status: 'Pending'
      });
      navigate('/thank-you');
    } catch (error) {
      console.error('Donation sync failed:', error);
      alert('Network directive failed. Intelligence synchronization interrupted. Please retry.');
    }
  };
  ...
  return (
    <div className="animate-in fade-in duration-700 bg-slate-50 min-h-screen">
      <SEO
        title="Donate Now"
        description="Support A.P.D.F.E's mission to empower women and children in Central Africa. Your donation provides education, healthcare, and safe spaces for survivors."
      />
