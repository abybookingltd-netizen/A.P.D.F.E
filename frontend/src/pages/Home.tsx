  ...
  return (
    <div className="animate-in fade-in duration-700">
        ...
        <div className="absolute bottom-10 flex gap-2 z-20">
          {HERO_SLIDES.map((_, idx) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentSlide ? 'w-8 bg-green-400' : 'w-2 bg-white/40'
                }`}
            />
          ))}
        </div>
      ...
      <section className="bg-white py-12 md:-mt-16 relative z-10 max-w-6xl mx-auto rounded-xl shadow-2xl px-8 grid grid-cols-2 md:grid-cols-4 gap-8">
        {[
          { label: "Lives Impacted", value: "5000+", icon: Heart, color: "text-red-500" },
          { label: "Active Programs", value: "25", icon: Activity, color: "text-blue-500" },
